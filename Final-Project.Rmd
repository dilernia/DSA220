...
An analysis report detailing your data, methods, results, and conclusions is part of the final group project. The report, including the R Markdown file and produced output is **due (submitted via Blackboard) by 11:59pm on Monday, December 11th**. A grading rubric which should be closely followed will be made available later in the semester. Please include comments throughout your R code so it is easier for me to read your code. If completing the final project in a group, an additional section must be included in the report detailing the contributions of each group member.
...
- At least one Monte Carlo simulation for conducting statistical inference of your data
...
```{r extractCounty, include = FALSE, eval = FALSE}
library(tidyverse)
library(sf)

# Extracting county info from addresses (not an easy task!)
countyInfo <- gunViolence %>% 
  separate(col = address_full, remove = FALSE, sep = ",", into = paste0("addr_info_", 1:24)) %>%
  dplyr::select(address_full, starts_with("addr_info_")) %>% 
  mutate(ID = 1:n()) %>% 
  pivot_longer(cols = starts_with("addr_info_"), names_to = "Component", values_to = "Value") %>%
  filter(str_detect(Value, pattern = "County|county")) %>% 
  group_by(ID) %>% 
  slice_tail(n = 1) %>% 
  ungroup()
```
...
**Number of observations**: `r nrow(gunViolence)`

**Number of variables**: `r ncol(gunViolence)`
...
dataDictionary(gunViolence, 
               descripts = c("geographic region ID",
...
"participant gender, one of 'Male' or 'Female'",
"participant name",
...
# Variables to query
queries <- tibble(Variable = c("NAME", 
                               "B01003_001", 
                               "B17001_002", 
                               "B17001_001",
                               "B19013_001", 
...
if(file.exists(paste0("census_data", minYear, "-", maxYear, ".csv")) == FALSE) {
# Get data for all counties in the United States for each year
county_data <- map_dfr(.x = (minYear:maxYear)[which(minYear:maxYear != 2020)],
                       .f = function(year) {
...
state_data <- map_dfr(.x = (minYear:maxYear)[which(minYear:maxYear != 2020)],
                       .f = function(year) {
...
# Pivoting
wide_census <- full_data %>% pivot_wider(id_cols = c(geoid, county_state, year),
                                         names_from = name, 
                                         values_from = value) %>% 
...
# Saving
write_csv(wide_census, file = paste0("census_data_", 
                                     min(wide_census$year), "-", max(wide_census$year), ".csv"))
} else {
  wide_census <- read_csv(paste0("census_data_", minYear, "-", maxYear, ".csv"))
}
```

```{r smallCensus, include = FALSE}
library(tidycensus)
library(tidyverse)
...
# Variables to query
queriesSmall <- tibble(Variable = c("NAME", 
                               "B01003_001", 
                               "B17001_002", 
                               "B17001_001",
                               "B19013_001", 
...
if(file.exists(paste0("census_data_small_", minYear, "-", maxYear, ".csv")) == FALSE) {
# Get data for all counties in the United States for each year
county_data_small <- map_dfr(.x = minYear:maxYear,
                       .f = function(year) {
                           get_acs(geography = "county", 
                                   survey = "acs5",
                                   variables = queriesSmall$Variable, 
                                   year = year) %>% 
                           mutate(year = year)
                       })

state_data_small <- map_dfr(.x = minYear:maxYear,
                       .f = function(year) {
                           get_acs(geography = "state", 
                                   survey = "acs5",
                                   variables = queriesSmall$Variable, 
...
# Merging
small_data <- county_data_small %>% 
  left_join(queriesSmall, by = c("variable" = "Variable")) %>% 
  dplyr::rename(value = estimate, 
                county_state = NAME) %>% 
  janitor::clean_names() %>% 
  dplyr::select(-moe, -label, -geography)
...
# Saving
write_csv(wide_small_census, file = paste0("census_data_small_", 
                                     min(wide_small_census$year), "-", max(wide_small_census$year), ".csv"))
} else {
  wide_small_census <- read_csv(paste0("census_data_small_", minYear, "-", maxYear, ".csv"))
}

# Importing expanded data
wide_census <- read_csv(paste0("census_data_", 2008, "-", maxYear, ".csv"))

# Combining detailed census data
full_census <- bind_rows(wide_census, wide_small_census) %>% 
  distinct(geoid, year, .keep_all = TRUE)

# Saving
write_csv(full_census, file = paste0("census_data_full_", 
                                     min(full_census$year), "-", max(full_census$year), ".csv"))
```

### Description

Estimates from the United States American Community Survey (ACS) at the county level for the years 2008 through 2021 based on an annual sample size of approximately 3.5 million addresses including data on population, income, poverty rates, and demographic characteristics, excluding the year 2020 as this data is unavailable. The data is contained in [census_data_full_2008-2021.csv](https://raw.githubusercontent.com/dilernia/STA418-518/main/Data/census_data_full_2008-2021.csv).
...
**Number of observations**: `r nrow(full_census)`

**Number of variables**: `r ncol(full_census)`

**Data Dictionary**:

```{r censusDataDict}
# Creating data dictionary
dataDictionary(full_census, 
               descripts = c("geographic region ID",
                             "geographic region",
"year",
"population",
...
2. Variables describing proportions: 
  ...
  a. **Gender**; `prop_female` and `prop_male`
...
## 3. Fast Food Restaurants Data

```{r, eval = FALSE, include = FALSE}
# Importing data with geoid now included
fastFoodGeo <- read_csv("fastFoodData/fastFoodGeo.csv")
```

```{r fetchFoodGeo, include = FALSE, eval = FALSE}
#| include: false
#| eval: false
...
# Saving
write_csv(fastFoodFull, file = "fastFoodData/fastFoodFull.csv")
```

```{r addFoodGeoID, eval = FALSE, include = FALSE}
library(tidyverse)
library(stringr)
library(lubridate)
library(ggpubr)
...
# Importing the data
fastFood <- read_csv("fastFoodData/fastFoodFull.csv")
...
# Creating sf data for distribution of restaurants
foodMapData <- county_shape %>% 
  full_join(full_census, by = c("GEOID" = "geoid")) %>% 
  dplyr::filter(year == 2016)

# Set coordinate reference system (CRS) of sf_food to match counties
st_crs(sf_food) <- st_crs(foodMapData)

# Merging with sf data
fastFoodGeo <- sf_food %>% 
  st_join(foodMapData) %>% as_tibble() %>% 
  dplyr::select(GEOID, keys) %>% 
  dplyr::rename(geoid = GEOID) %>% 
  distinct() %>% 
  full_join(fastFood)

# Saving with geoid now included
write_csv(fastFoodGeo, 
          file = "fastFoodData/fastFoodGeo.csv")

# Subsetting sf_food data for plotting
sf_food_2016 <- foodMapData %>% 
  dplyr::filter(year == 2016) %>% 
  distinct() %>% 
  full_join(
  fastFoodGeo %>%
  count(geoid) %>% 
  dplyr::rename(n_restaurants = n), by = c("GEOID" = "geoid")) %>% 
  mutate(restaurants_per_capita = n_restaurants / population*100000) %>% 
    tigris::shift_geometry()

# Creating choropleth map
foodGG <- sf_food_2016 %>% 
  ggplot(aes(fill = restaurants_per_capita)) +
  geom_sf() +
  scale_fill_gradient(low = "white", high = "dodgerblue") +
  labs(title = "County-level restaurants per capita", fill = "Restaurants per 100,000 people") +
  theme_void()
...
This is a list of 10,000 fast food restaurants provided by [Datafiniti's Business Database](https://datafiniti.co/products/business-data/). The dataset includes the restaurant's address, city, latitude and longitude coordinates, name, and other characteristics. The data is contained in [fastFoodGeo.csv](https://raw.githubusercontent.com/dilernia/STA418-518/main/Data/fastFoodGeo.csv).

### Source

Data was obtained from the [Kaggle website](https://www.kaggle.com/datasets/datafiniti/fast-food-restaurants).

### Attributes

**Number of observations**: `r nrow(fastFoodGeo)`

**Number of variables**: `r ncol(fastFoodGeo)`

**Data Dictionary**:

```{r}
# Creating data dictionary
dataDictionary(fastFoodGeo, 
               descripts = c("geographic region ID",
                             "address",
"city",
"country",
...
1. MIT Election Data and Science Lab, 2017, "U.S. President 1976–2020", https://doi.org/10.7910/DVN/42MVDX, Harvard Dataverse, V6, UNF:6:4KoNz9KgTkXy0ZBxJ9ZkOw== [fileUNF]

### Attributes

**Number of observations**: `r nrow(state_pres)`

**Number of variables**: `r ncol(state_pres)`
...
dataDictionary(state_pres, 
               descripts = c('election year', 'state name', 'state postal code abbreviation', 'state fips code', 'state census code', 'state Inter-university Consortium for Political and Social Research code', 'name of the public office to which the candidate is seeking election', 'candidate name', "full name of the candidate's political party", 'candidate is a write-in (TRUE or FALSE) ', 'number of votes for candidate', 'total votes cast in the election', 'version', 'notes', 'just the major parties, with others marked as "other"'))
```
...
1. MIT Election Data and Science Lab, 2018, "County Presidential Election Returns 2000-2020", https://doi.org/10.7910/DVN/VOQCHQ, Harvard Dataverse, V11, UNF:6:HaZ8GWG8D2abLleXN3uEig== [fileUNF]

### Attributes

**Number of observations**: `r nrow(county_pres)`

**Number of variables**: `r ncol(county_pres)`
...
dataDictionary(county_pres, 
               descripts = c('year of election', 'state name', 'state postal code abbreviation', 'county name', 'county fips code', 'name of the public office to which the candidate is seeking election', 'candidate name', 'party', 'number of votes for candidate', 'total votes cast in the election', 'version', 'mode'))
```
